...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C2f_Faster', 'C3_Faster', 'C3_ODConv', 'C2f_ODConv', 'Partial_conv3', 'C2f_Faster_EMA', 'C3_Faster_EMA', 'C2f_DBB',
           ...
           'C3_UniRepLKNetBlock', 'C2f_UniRepLKNetBlock', 'C3_DRB', 'C2f_DRB', 'C3_DWR_DRB', 'C2f_DWR_DRB', 'Zoom_cat1','Zoom_cat', 'ScalSeq', 'DynamicScalSeq','DynamicScalSeq1','DynamicScalSeq2','DynamicScalSeq3','Add', 'CSP_EDLAN', 'asf_attention_model',
...
class DynamicScalSeq1(nn.Module):
    def __init__(self, inc, channel):
        super(DynamicScalSeq1, self).__init__()
        if channel != inc[0]:
            self.conv0 = Conv(inc[0], channel, 1)
        self.conv1 = Conv(inc[1], channel, 1)
        self.conv2 = Conv(inc[2], channel, 1)
        ...
        self.dysample1 = DySample(channel, 1, 'lp')
        self.dysample2 = DySample(channel, 2, 'lp')
...
class DynamicScalSeq2(nn.Module):
    def __init__(self, inc, channel):
        super(DynamicScalSeq2, self).__init__()
        if channel != inc[0]:
            self.conv0 = Conv(inc[0], channel, 1)
        self.conv1 = Conv(inc[1], channel, 1)
        self.conv2 = Conv(inc[2], channel, 1)
        ...
        self.dysample1 = CARAFE(c=64,scale=2)
        self.dysample2 = CARAFE(c=64,scale=4)
...
class DynamicScalSeq3(nn.Module):
    def __init__(self, inc, channel):
        super(DynamicScalSeq3, self).__init__()
        if channel != inc[0]:
            self.conv0 = Conv(inc[0], channel,1)
        self.conv1 =  Conv(inc[1], channel,1)
        self.conv2 =  Conv(inc[2], channel,1)
    ...
    def forward(self, x):
        ...
        if hasattr(self, 'conv0'):
            p3 = self.conv0(p3)
        p4_2 = self.conv1(p4)
        p4_2 = F.interpolate(p4_2, p3.size()[2:], mode='bilinear')
        p5_2 = self.conv2(p5)
        p5_2 = F.interpolate(p5_2, p3.size()[2:], mode='bilinear')
        p3_3d = torch.unsqueeze(p3, -3)
        p4_3d = torch.unsqueeze(p4_2, -3)
        p5_3d = torch.unsqueeze(p5_2, -3)
        combine = torch.cat([p3_3d, p4_3d, p5_3d],dim = 2)
